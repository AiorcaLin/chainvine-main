...
| Slither | v0.10.x（Docker 容器） |
| AI 模型 | DashScope qwen-flash / qwen-plus |
| 浏览器 | Chrome 最新版 |
...
| 合约 | 地址 | 行数 | 特点 |
|------|------|------|------|
| USDT (Tether) | `0xdAC17...ec7` | ~500 | 单文件 ERC-20，经典实现 |
| 内置 Vault 示例 | — | ~116 | 包含已知重入漏洞 |

> **说明**：受限于时间和 API 成本，测试规模有限。更大规模的对比实验列为未来工作。

### 4.3 实验设计

**实验目标**：对比以下三种检测方式在相同合约上的表现：
1. **Slither 单引擎**：仅使用 Slither 静态分析
2. **AI 单引擎**：仅使用 LLM 深度审计（DashScope qwen-flash）
3. **双引擎融合**：Slither + AI + 融合引擎

**评估指标**：
- 检出数量（Total Findings）
- 严重度分布（Critical / High / Medium / Low / Gas）
- 交叉验证数量（Cross-Validated）
- 分析耗时（Duration）

### 4.4 实验结果

（此处需要在实际运行后填入数据。以下为框架示例。）
...
1. **互补性验证**：Slither 检测到但 AI 遗漏的漏洞类型，以及 AI 检测到但 Slither 遗漏的类型
2. **交叉验证效果**：双引擎同时发现的漏洞数量及其特征
3. **误报分析**：各引擎的假阳性情况
4. **性能对比**：双引擎并行执行的时间效率
