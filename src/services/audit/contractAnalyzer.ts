...
export async function analyzeContract(params: {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      formattedResponse =
        "# Generated by [ChainVine](https://chainvine.vercel.app/)\n\n" +
        formattedResponse;
      ...
      // ── 不可恢复错误：立即抛出，不重试 ──
      // 402 Payment Required（余额不足）、401 Unauthorized（Key 无效）、403 Forbidden
      // 参考 RFC 9110 §15.5: 4xx 客户端错误通常不应重试
      const errMsg = error instanceof Error ? error.message : "";
      if (errMsg.includes("402") || errMsg.includes("401") || errMsg.includes("403")) {
        console.error(`Non-retryable API error: ${errMsg}`);
        throw error;
      }
